...
class Builder
    ...
    def buildModules
        ...
        @worker.run("xcodebuild -project OCMock.xcodeproj -target OCMockLib -sdk iphoneos8.4 OBJROOT=#{@env.objroot} SYMROOT=#{@env.symroot}")
        @worker.run("xcodebuild -project OCMock.xcodeproj -target OCMockLib -sdk iphonesimulator8.4 OBJROOT=#{@env.objroot} SYMROOT=#{@env.symroot}")
